...
class Config:
  ...
  prior_dir: str = None # Prior data directory, just for p2nerf
  ...
  replica_scene: str = 'room0'  # Which replica scene to use.
  ...
  # Keypoints loss
  kpts_loss_mult: float = 0.0
  kpts_patch_size: int = 16
  kpts_init_iters: int = 512
  kpts_early_train_every: int = 3
  kpts_weight_mask_ratio: float = 0.2
  kpts_weight_mask_weight: float = 1.0
  # Depth loss
  depth_loss_mult: float = 0.0
  depth_rank_level: int = 32
  depth_weight_mask_margin: float = 0.1
  depth_rank_weight: float = 1.0
  depth_weight_mask_weight: float = 1.0
