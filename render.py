...
def create_videos(config, base_dir, out_dir, out_name, num_frames):
  ...
  depth_file = os.path.join(out_dir, f'distance_mean_000.tiff')
  ...
  for k in ['color', 'normals', 'acc', 'distance_mean', 'distance_median']:
    ...
    idx = 0
    file0 = os.path.join(out_dir, f'{k}_000.{file_ext}')
    ...
    with media.VideoWriter(
      ...
      for idx in range(num_frames):
        img_file = os.path.join(out_dir, f'{k}_{idx:03d}.{file_ext}')
...
def main(unused_argv):
  ...
  # elif config.dataset_loader == 'replica_prior':
  #   lo, hi = dataset.near, dataset.far
