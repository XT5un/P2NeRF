...
class TrainStats:
  ...
  losses_kpts_weight: float
  kpts_depth_errs: float
  # losses_kpts_pair: float
  losses_depth_rank: float
  losses_depth_weight: float
...
def train_step(
  ...
  def loss_fn(variables):
    ...
    ## -------------------------------------------------------------- ##
        ...
        compute_extras=(config.compute_disp_metrics or
                        ...
                        config.depth_loss_mult > 0. or
                        config.kpts_loss_mult > 0.),
    ...
    for rendering, ray_hist in zip(renderings, ray_history):
      numer = (lossmult * (rendering['rgb'] - batch['rgb'][Ellipsis, :3])**2).sum()
      denom = lossmult.sum()
      losses.append(numer / denom)

      # compute kpts loss
      if config.kpts_loss_mult > 0. and 'kpts_depth' in batch:
        if config.kpts_weight_mask_weight > 0.:
          losses_kpts_weight.append(math.lossfunc_kpts_weight_mask(ray_hist['weights'], ray_hist['t_vals'], batch['kpts_depth'],
                                                                   batch['rays'].near, batch['rays'].far,
                                                                   config.kpts_weight_mask_ratio, config.kpts_patch_size))
          err = math.get_kpts_depth_error(rendering['distance_mean'], batch['kpts_depth'], config.kpts_patch_size, 'L1')
          kpts_depth_errs.append(err)
        else:
          losses_kpts_weight.append(0.0)
          kpts_depth_errs.append(0.0)
      else:
        losses_kpts_weight.append(0.0)
        kpts_depth_errs.append(0.0)

      # compute depth loss
      if config.depth_loss_mult > 0.:
        if config.depth_rank_weight > 0.:
          rank_loss = math.lossfunc_depth_rank(rendering['distance_mean'], config.depth_rank_level)
          losses_depth_rank.append(rank_loss)
        else:
          losses_depth_rank.append(0.0)
        if config.depth_weight_mask_weight > 0.:
          losses_depth_weight.append(
            math.lossfunc_depth_weight_mask(rendering['distance_mean'], ray_hist['weights'], ray_hist['t_vals'],
                                            config.depth_rank_level, config.depth_weight_mask_margin))
        else:
          losses_depth_weight.append(0.0)
      else:
        losses_depth_rank.append(0.0)
        losses_depth_weight.append(0.0)
    ...
    losses_kpts_weight = jnp.array(losses_kpts_weight)
    # losses_kpts_pair = jnp.array(losses_kpts_pair)
    losses_depth_rank = jnp.array(losses_depth_rank)
    losses_depth_weight = jnp.array(losses_depth_weight)
    kpts_depth_errs = jnp.array(kpts_depth_errs)
    loss = (
        # fine color loss
        losses[-1] + \
        # coarse color loss
        config.coarse_loss_mult * jnp.sum(losses[:-1]) + \
        # weight l2
        config.weight_decay_mult * weight_l2 + \
        # kpts weight mask loss
        config.kpts_loss_mult * config.kpts_weight_mask_weight * losses_kpts_weight[-1] + \
        config.kpts_loss_mult * config.kpts_weight_mask_weight * jnp.sum(losses_kpts_weight[:-1]) + \
        # depth rank loss
        config.depth_loss_mult * config.depth_rank_weight * losses_depth_rank[-1] + \
        config.depth_loss_mult * config.depth_rank_weight * jnp.sum(losses_depth_rank[:-1]) + \
        # depth weight mask loss
        config.depth_loss_mult * config.depth_weight_mask_weight * losses_depth_weight[-1] + \
        config.depth_loss_mult * config.depth_weight_mask_weight * jnp.sum(losses_depth_weight[:-1]) + \
        # regnerf georeg loss
    ...
    return loss, (losses, disp_mses, normal_maes, weight_l2, losses_georeg,
                  losses_kpts_weight, kpts_depth_errs,
                  losses_depth_rank, losses_depth_weight)

  (loss, loss_aux), grad = (jax.value_and_grad(loss_fn, has_aux=True)(
      state.optimizer.target))
  (losses, disp_mses, normal_maes, weight_l2, losses_georeg, 
   losses_kpts_weight, kpts_depth_errs,
   losses_depth_rank, losses_depth_weight) = loss_aux
  ...
  losses_kpts_weight = jax.lax.pmean(losses_kpts_weight, axis_name='batch')
  # losses_kpts_pair = jax.lax.pmean(losses_kpts_pair, axis_name='batch')
  losses_depth_rank = jax.lax.pmean(losses_depth_rank, axis_name='batch')
  kpts_depth_errs = jax.lax.pmean(kpts_depth_errs, axis_name='batch')
  losses_depth_weight = jax.lax.pmean(losses_depth_weight, axis_name='batch')
  ...
  stats = TrainStats(
      ...
      losses_kpts_weight=losses_kpts_weight,
      kpts_depth_errs=kpts_depth_errs,
      # losses_kpts_pair=losses_kpts_pair,
      losses_depth_rank=losses_depth_rank,
      losses_depth_weight=losses_depth_weight,
  )
...
def main(unused_argv):
  ...
  for step, batch in zip(range(init_step, config.max_steps + 1), pdataset):
    ...
    if jax.host_id() == 0:
      ...
      # kpts初始阶段每个step都保存日志
      is_report_kpts = False
      if config.kpts_loss_mult > 0.:
        if step <= config.kpts_early_train_every * config.kpts_init_iters and step % config.kpts_early_train_every == 0:
          is_report_kpts = True
        if is_report_kpts:
          for i, l in enumerate(stats.losses_kpts_weight):
              summary_writer.scalar(f'train_losses_kpts_weight_{i}', l, step)
          for i, l in enumerate(stats.kpts_depth_errs):
            summary_writer.scalar(f'train_anchor_depth_errs_{i}', l, step)
      ...
      if step % config.print_every == 0:
        ...
        if config.depth_loss_mult > 0.:
          for i, l in enumerate(stats.losses_depth_rank):
            summary_writer.scalar(f'train_losses_depth_rank_{i}', l, step)
          for i, l in enumerate(stats.losses_depth_weight):
            summary_writer.scalar(f'train_losses_depth_weight_{i}', l, step)
        if config.compute_disp_metrics:
